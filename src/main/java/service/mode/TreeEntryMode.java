...
public record TreeEntryMode(TreeEntryModeType type, int permission) {

    public TreeEntryMode {
        if (type.isPermissionless() && permission != 0) {
            throw new IllegalArgumentException("%s is permissionless, but permission is %d".formatted(type, permission));
        }
    }
