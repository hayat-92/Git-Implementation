...
public class CommitSerializer implements ObjectContentSerializer<Commit> {
    ...
    public static final Pattern AUTHOR_PATTERN =
            Pattern.compile("^(.*?) <(.*?)> (\\d+) ((?:\\+|-)\\d+)$");
    @Override
    public void serialize(Commit commit, DataOutputStream dataOutputStream)
            throws IOException {
    ...
    public static void serializeAuthor(byte[] keyBytes, AuthorSignature author,
    ...
    @Override
    public Commit deserialize(DataInputStream dataInputStream)
            throws IOException {
